...
ods_payment = PostgresOperator(
    ...
    # Для больших данных нужна партицированная таблица по date_part_year и замена
    # DELETE на TRUNCATE партиции.
    sql="""
        DELETE FROM asamoilov.ods_payment WHERE date_part_year = {{ execution_date.year }};
    ...
    sql="""
        ...
        WITH source_data AS (
            SELECT s.billing_period_pk, 
                ...
                s.record_source
            FROM (
                SELECT p.billing_period_pk, 
                    p.billing_period_key, 
                    p.load_date, 
                    p.record_source,
    ...
    sql="""
        ...
        WITH source_data AS (
            SELECT p.user_pk,
                ...
                p.record_source,
                CASE
                    WHEN lag(p.user_hashdiff, 1, 'none') OVER (PARTITION BY p.user_pk ORDER BY p.effective_from) = p.user_hashdiff
                        THEN 'N'
                    ELSE 'Y'
                END AS is_update
            FROM asamoilov.ods_payment_v p
        ...
        LEFT JOIN update_records ur1 ON sd1.user_pk = ur1.user_pk
            AND sd1.user_hashdiff = ur1.user_hashdiff
...
ods_payment >> dds_sat_user_details >> all_loaded
